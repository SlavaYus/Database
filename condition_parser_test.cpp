...
void TestParseCondition() {
    {
        istringstream is("date != 2020-11-03");
        shared_ptr<Node> root = ParseCondition(is);
        Assert(root->Evaluate({ 2021, 2, 8 }, ""), "Parse condition 1");
        Assert(!root->Evaluate({ 2020, 11, 3 }, ""), "Parse condition 2");
    }
    {
        istringstream is(R"(event == "sport event")");
        shared_ptr<Node> root = ParseCondition(is);
        Assert(root->Evaluate({ 2020, 1, 7 }, "sport event"), "Parse condition 3");
        Assert(!root->Evaluate({ 2021, 2, 8 }, "holiday"), "Parse condition 4");
    }
    {
        istringstream is("date >= 2020-01-01 AND date < 2020-07-01");
        shared_ptr<Node> root = ParseCondition(is);
        Assert(root->Evaluate({ 2021, 1, 1 }, ""), "Parse condition 5");
        Assert(root->Evaluate({ 2020, 3, 1 }, ""), "Parse condition 6");
        Assert(root->Evaluate({ 2020, 6, 30 }, ""), "Parse condition 7");
        Assert(!root->Evaluate({ 2020, 7, 1 }, ""), "Parse condition 8");
