...
void TestDateComparisonNode() {
    const Date november_1{ 2020, 11, 1 };
    const Date november_2{ 2020, 11, 2 };
    const Date november_3{ 2020, 11, 3 };
...
void TestLogicalOperationNode() {
    ...
    }
    {
        LogicalOperationNode root(
            ...
            make_shared<DateComparisonNode>(Comparison::Equal, Date{ 2020, 11, 18 }),
            make_shared<EventComparisonNode>(Comparison::Equal, "Saturday")
        );

        Assert(root.Evaluate({ 2020, 11, 18 }, "Saturday"), "LogicalOperationNode propagates arguments 1");
        Assert(!root.Evaluate({ 2020, 11, 18 }, "Friday"), "LogicalOperationNode propagates arguments 2");
        Assert(!root.Evaluate({ 2020, 11, 1 }, "Saturday"), "LogicalOperationNode propagates arguments 1");
    }
}